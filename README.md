...
初期表示は English です。ライブラリ画面の言語セレクタ、またはアプリの **Settings... / 設定...** から **日本語** に切り替えられます。Reader 画面には、表示崩れを避けるため言語セレクタを置いていません。
...
Windows 版は現在、実験的対応です。macOS 版ほど成熟した配布形態ではありませんが、Windows 11 上での起動と基本的な読書機能を確認しています。

確認済みの環境:

- Intel iMac 2020 上の VirtualBox で動作する Windows 11
- `Perseus Local Reader.bat` からの起動
- bundled Python runtime の自動セットアップ
- `pywebview` による WebView shell の起動
- ローカルサーバーの `/api/works` 応答
- `reader.html` の表示

Windows で使う場合:

1. GitHub から ZIP をダウンロードします。
2. ZIP を必ず通常のフォルダに展開します。
3. 展開後の `perseus-local-reader-main/` フォルダを開きます。
4. `Perseus Local Reader.bat` を実行します。
5. 初回起動時は、bundled Python runtime と依存パッケージのセットアップが走るため、数分かかる場合があります。

注意:

- ZIP の中を直接開いた状態、または `AppData\Local\Temp\...zip...` のような一時展開パスからは実行しないでください。
- 必ず ZIP を展開した通常フォルダから `Perseus Local Reader.bat` を実行してください。
- 初回セットアップではインターネット接続が必要です。
- Windows 11 では Microsoft Edge WebView2 Runtime が通常は既に入っています。見つからない場合は、Windows 側で WebView2 Runtime の導入が必要です。
- Windows 版は現在 experimental です。問題が出た場合は、まず `.developer/data/build/` 以下のログを確認してください。
...
上記は macOS 版(`Perseus Local Reader.app`)の説明です。Windows では代わりに `Perseus Local Reader.bat` を使います。仕組みの詳細は `.developer/windows-app/DESIGN.md` を参照してください。

対応環境:

- Windows 10 または Windows 11(64bit)
- 初回起動時はインターネット接続が必須です(Python ランタイムを自動セットアップするため)
- Microsoft Edge WebView2 Runtime(Windows 11 にはプリインストール済みです。Windows 10 では入っていない場合があります)
...
2. ZIP を解凍します。
3. 解凍された `perseus-local-reader-main/` フォルダを、丸ごと好きな場所に置きます。
4. `Perseus Local Reader.bat` をダブルクリックします。
5. 初回起動時のみ、自動セットアップ(Python ランタイムのダウンロードと依存関係のインストール)が走ります。インターネット接続の速さにもよりますが数分かかることがあるので、ウィンドウを閉じずに待ちます。
6. セットアップが終わるとそのまま Reader が起動します。2 回目以降はセットアップをスキップしてすぐに起動します。

想定される最上位構成は macOS 版と同じで、`Perseus Local Reader.app` の代わりに `Perseus Local Reader.bat` が並びます。

初回起動で SmartScreen に止められた場合:

このアプリは Windows 向けのコード署名をしていません。「Windows によって PC が保護されました」という SmartScreen の警告が出た場合は、**詳細情報** をクリックし、**実行** を選んでください。

トラブルが起きた場合:

- セットアップ・ローカルサーバーのログは `.developer/data/build/pyshell-server-*.log` です。
- `.developer/data/vendor/windows-runtime/` フォルダを削除すると、次回起動時に Python ランタイムが最初から再セットアップされます。作品データ・語形解析・メモは別の場所に保存されているため、このフォルダを削除しても消えません。
...
Windows uses a separate, pywebview-based shell instead of the Swift/AppKit controller described above. The Python backend (`server.py`, `text_store.py`, `tei_convert.py`, `fetch_morph.py`) and the web UI under `.developer/app` are shared, unmodified, between macOS and Windows. Full design and rationale live in `.developer/windows-app/DESIGN.md`.

Components:

```text
Perseus Local Reader.bat        Windows launcher (double-click entry point)
.developer/windows-app/
+-- DESIGN.md                   Windows design document
+-- shell.py                    pywebview shell (cross-platform; also runnable on macOS for development)
+-- bootstrap.ps1               First-run Python runtime bootstrap
+-- requirements.txt            Pinned pywebview dependency
.developer/data/vendor/windows-runtime/python/   Bootstrapped embeddable Python (gitignored; preserved across updates)
```

Startup flow:

1. `Perseus Local Reader.bat` resolves the repository root from its own location (`%~dp0`) so the extracted folder can live anywhere, including a path containing spaces.
2. If `.developer/data/vendor/windows-runtime/python/pythonw.exe` does not exist yet, the `.bat` runs `powershell -NoProfile -ExecutionPolicy Bypass -File ".developer\windows-app\bootstrap.ps1"`. On failure it prints the error and pauses instead of closing.
3. `bootstrap.ps1` downloads a pinned Python 3.12 embeddable package (amd64) from python.org, verifies its SHA-256, extracts it, enables `site-packages` in `python312._pth`, bootstraps `pip` via `get-pip.py`, and installs the pinned `pywebview` version from `requirements.txt`. It is idempotent and safe to re-run after a partial failure (downloads go to a temporary file, are verified, then renamed/extracted). It also checks the registry for the Microsoft Edge WebView2 Runtime and prints bilingual guidance (warning only, never a hard failure) if it is missing.
4. Once the runtime is ready, the `.bat` starts `pythonw.exe .developer/windows-app/shell.py` with no console window and closes itself.

`shell.py` mirrors the macOS controller contract (root detection, `server.py` launch with `--parent-pid`, port `8000`-`8010` probing, `shell_settings.json` in place of `UserDefaults`, embedded webview vs. default-browser open targets) but relies on `pywebview` instead of AppKit/WebKit. See `.developer/windows-app/DESIGN.md` §3 for the full macOS-to-Windows API mapping and §7 for the outstanding Windows hardware verification checklist.

`requirements.txt` pins `pywebview==6.2.1`. The shell's use of `webview.menu.Menu`/`MenuAction`/`MenuSeparator`, `create_window(text_select=, js_api=)`, `start(menu=)`, `window.events.loaded`, `get_current_url`, `load_url`, and `evaluate_js` has been verified against that version.
...
Perseus Local Reader now includes an experimental Windows launcher.

Current verified status:

- Tested on Windows 11 running in VirtualBox on an Intel iMac 2020 host.
- `Perseus Local Reader.bat` bootstraps a bundled Windows Python runtime under `.developer/data/vendor/windows-runtime/python`.
- The bootstrap installs the pinned Python dependencies from `.developer/windows-app/requirements.txt`.
- The Windows runtime uses `pywebview` for the native WebView shell.
- The local Python server has been verified on Windows:
  - `GET /api/works` returns the downloaded work list.
  - `reader.html?urn=urn:cts:greekLit:tlg0059.tlg001` returns HTTP 200.
- The app has been manually launched on Windows and confirmed to display the reader UI.

Important notes:

- Do not run `Perseus Local Reader.bat` directly from inside a ZIP preview.
  Extract the ZIP first, then run the `.bat` file from the extracted folder.
- On first launch, the bootstrap may take several minutes because it downloads
  the Python embeddable runtime, installs `pip`, and installs the required Python packages.
- If setup fails because `pywebview` is missing, rerun `Perseus Local Reader.bat`
  from the extracted project folder.
- Windows support is currently experimental and should be treated as less mature
  than the macOS app bundle.

Implementation notes:

- `.developer/windows-app/bootstrap.ps1` prepares the bundled Windows Python runtime.
- `.developer/windows-app/shell.py` starts the local reader server and opens the WebView shell.
- `.developer/scripts/server.py` explicitly adds its own script directory to `sys.path`
  so that adjacent modules such as `text_store.py` are importable under Windows embeddable Python.
